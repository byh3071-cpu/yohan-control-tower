...
- **안 만들면**: 노션 AI 워크스페이스·요한 스튜디오 OS처럼 복잡성이 올라가 블랙박스가 되고 → **결국 안 쓴다**(이미 발생). 그리고 **"관제탑"이 두 실체**(`yohan-control-tower` 벡터 도구 / `yohan-brain/dashboard` 관제 화면)를 가리키는 상태가 유지된다. 감사 H-02 는 2026-07-19 재리뷰에서 소유 경로를 `brain/dashboard` 로 **고정해 해소**했지만(`ECOSYSTEM-MASTER-PLAN-REVIEW-2026-07-19.md:177`), 실체는 여전히 둘이라 같은 충돌이 재발할 구조다. **v1.0 이 실체를 하나로 만든다** (ADR-012).
...
- **v1.0 완료 정의**: 생태계에서 "관제탑"이 가리키는 실체가 **1개**(계약·registry·문서·코드 전부) + 기존 6탭 기능 무손실 이관 + 4탭 기동.
