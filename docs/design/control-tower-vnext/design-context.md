...
- **Observed 2026-08-24** — 디자인 Goal 21는 production UI를 수정하지 않고 종료했으며, 별도로 승인된 Goal 22에서 NOW-R3 첫 구현 슬라이스를 완료했다.
...
| `src/components/now-view.tsx`, `src/app/api/todos/route.ts` | Goal 22 verified working tree | NOW-R3 구현과 `projects.yaml → repo/goals` 읽기 계약 | project internal | NOW 데이터 계약 또는 Home 변경 시 |
...
NOW-R3는 정적 승인 뒤 Goal 22에서 실제 Goal 데이터와 360·432·768·1280·1440px, loading·error·empty·single·multiple 상태, 키보드, 대비, 경로 allowlist를 검증했고 `design-qa.md`가 passed로 끝났다. 권장 순서 2번인 `작업`의 할 일·일정·프로젝트 형제 보기는 2026-08-25에 Goal 23으로 승인되어 구현·검증 중이다.
