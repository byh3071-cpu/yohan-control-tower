...
- 의도적 차이: prototype의 가상 vNext 전역 탐색은 노출하지 않고 현재 production 5탭을 유지했다. 데이터는 예시 Goal 15가 아니라 실제 local active Goal 22를 사용했다.
...
- 기존 Next NFT 과추적 경고 1건은 `next.config.ts → memory.ts → search route` 경로이며 Goal 16 변경과 무관하다. build는 성공했다.
