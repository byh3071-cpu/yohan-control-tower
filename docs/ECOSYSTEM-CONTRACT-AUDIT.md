...
Home UI 셸은 `홈 · 프로젝트 · 문서 · 기록 · 벡터` 5탭, Brain 우선, 미연결 상태 명시, 사람 승인 원칙에 맞는다. GitHub Connector로 active 계약 v0.3.0·`projects.yaml` v0.1.1·ADR-013 Accepted를 확인했고, F004 미션 롤업과 실제 브라우저 QA까지 완료했다. PR은 자동 병합하지 않고 사람 검토를 위한 Draft로 유지한다.

## 역할 계약

| 영역 | 합의된 역할 | 현재 MVP | 판정 |
| --- | --- | --- | --- |
| Control Tower | 사람용 통합 관제·판단·승인 | Home 미션 롤업과 기존 4개 원장으로 진입점 제공 | 일치 |
...
| VHK | `DEFINE → EXECUTE → VERIFY` 실행·검증 | Goal 2·품질 게이트·증거 로그 사용, 다레포 실행 집계 없음 | 부분 |
...
- Home의 미션 롤업은 연결됐지만 프로젝트 3단 드릴다운, AI 완료, 승인 대기 건수, 생태계 블로커 통합은 아직 공통 집계 API가 없다.
...
4. **브라우저 QA — 해결**: Playwright 1440×900·390×844에서 가로 overflow 0, 탭 5개, 미션 카드 5개, 카드→프로젝트 이동, 콘솔 오류 0 확인.
...
- F005 미션→프로젝트→Task 드릴다운, F006 정합성 lint
...
1. Draft PR에서 Home·F004 미션 롤업·기본 Light 디자인을 사람 눈으로 검토한다.
2. F005 미션→프로젝트→Task 드릴다운을 한 세로 슬라이스로 구현한다.
3. F006 정합성 lint를 연결해 Home에 결함 수만 노출한다.
4. 이후 Calendar 또는 Skill Registry 중 하나만 선택한다. Finance는 거래 원장 계약 뒤에 시작한다.
