...
- AI 실행과 재무는 데이터 계약이 없음을 화면에 드러낸다. Calendar는 ADR-002의 로컬 Markdown 계약을 사용한다.
...
| P1 | 모바일·PWA | Tailscale 전용인지 설치형 PWA인지, OS 알림을 로컬 서버와 Service Worker 중 누가 실행할지 |
| P2 | Notion 리모델링 | Mirror/Registry/Inbox만 남길 때 보존할 DB와 읽기 전용 전환 순서 |

## 명시적 비범위

- 기존 Notion 일정 337행 마이그레이션, 외부 Calendar 양방향 동기화, OS/PWA 알림
