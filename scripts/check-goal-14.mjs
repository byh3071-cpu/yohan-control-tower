...
// ─── goal 14 고유 검증 (직접 추가) ───────────────────────────────
must(pkg.dependencies?.next === '16.3.2', 'next가 16.3.2로 고정됨')
must(pkg.dependencies?.['@next/env'] === '^16.3.2', '@next/env가 16.3.2 계열로 정렬됨')
must(pkg.devDependencies?.['eslint-config-next'] === '16.3.2', 'eslint-config-next가 16.3.2로 정렬됨')
must(pkg.overrides?.postcss === '8.5.26', 'postcss override가 8.5.26으로 고정됨')
gate('npm audit', run('npm', ['audit', '--audit-level=low']))
