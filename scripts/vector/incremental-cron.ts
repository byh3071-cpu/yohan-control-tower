 ...
 *
 * 실행: npx tsx scripts/vector/incremental-cron.ts   (cwd = control-tower 루트)
 *
 * 주의: vercel.json 은 제거됐다 — cron 이 GET 으로 치는데 라우트는 POST 만 export 해서
 * 처음부터 동작하지 않았다(ADR-012). 이 스크립트 + Windows 작업스케줄러가 유일한 증분 경로.
 */
...
async function main() {
  ...
  const { ingestAllIncremental } = await import('../../src/lib/vector/ingest')
