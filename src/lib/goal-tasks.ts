...
export function stableTodoId(relPath: string, text: string, discriminator?: string | number): string {
  const identity = discriminator === undefined ? text : `${text}\n${String(discriminator)}`
...
export function parseGoalCompletionTasks(
  ...
  const items = checks.flatMap<TodoItem>((check, index) => {
    ...
    return [{
      id: stableTodoId(relPath, check.text, check.line),
      text: check.text,
      relPath,
      line: check.line,
