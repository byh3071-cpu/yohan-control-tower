...
/**
 * 벡터 인프라 패널 — 노션 SoT → Qdrant 복사본 관제(F003).
 *
 * 페이지가 아니라 컴포넌트인 이유: 탭 셸(`app/page.tsx`)의 벡터 탭과
 * 독립 주소(`/vector`)가 **같은 화면을 공유**해야 하기 때문이다.
 * 어느 한쪽에만 두면 다른 쪽이 빈 껍데기가 되거나 코드가 갈라진다.
 */
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-6">
      <section className="mb-6">
        <h2 className="mb-2 text-sm font-semibold text-foreground">Qdrant 상태</h2>
        <CollectionStatus status={status} stale={statusStale} updatedAt={statusUpdatedAt} />
      </section>

      {TIERS.map((g) => (
        <section key={g.tier} className="mb-5">
          <h2 className="mb-2 text-sm font-semibold text-foreground">{g.title}</h2>
      ...
      <section className="mb-6">
        <h2 className="mb-2 text-sm font-semibold text-foreground">검색 테스트</h2>
        <QueryTester onSearch={search} result={queryResult} searching={searching} error={queryError} />
      </section>

      <section>
        <h2 className="mb-2 text-sm font-semibold text-foreground">실시간 로그</h2>
